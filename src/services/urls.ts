...
export const createShortURL = async (
  ...
  if (body.id) {
    ...
    if (current_record) {
      throw new httpErrors.Conflict("The ID that you provided already exists");
    }
  }
...
export const resolveURL = async (id: string, ip: string) => {
  ...
  if (!url) {
    throw new httpErrors.NotFound("The URL is not found");
  }
...
export const getURLS = async (
  ...
  const results = await knex("urls")
    ...
    .select(['urls.id', 'urls.url', 'urls.created_at', knex.raw('count(visits.id) as visits_count')])
    .limit(limit || 15)
    .offset(offset || 0)
    .groupBy('urls.id')
